...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    var x = body.x;
    var y =body.y;
    if(x<boundaries.minX||boundaries.minX.isNaN())
      boundaries.minX=x
    else if(x>boundaries.maxX||boundaries.maxX.isNaN())
      boundaries.maxX=x
     if(y<boundaries.minY||boundaries.minY.isNaN())
       boundaries.minY=y
     else if(y>boundaries.maxY||boundaries.maxY.isNaN())
       boundaries.maxY=y
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    return parBodies.aggregate(new SectorMatrix(boundaries,barneshut.SECTOR_PRECISION))((x,y)=>(x.+=(y)),(a,b)=>(a.combine(b)));
  }
